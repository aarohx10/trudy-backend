-- Fix audit trigger function to be resilient to NULL client_id
-- This allows inserts to proceed even if client_id cannot be determined (e.g. testing with NULL clerk_org_id)
...
BEGIN
    ...
    -- CRITICAL FIX: If client_id is still NULL, SKIP AUDIT LOG instead of failing transaction
    -- This prevents P0001 errors when creating drafts without full context
    IF v_client_id IS NULL THEN
        RAISE WARNING 'Skipping audit log for % on %: Cannot determine client_id (clerk_org_id: %)', TG_OP, TG_TABLE_NAME, v_clerk_org_id;
